...
**Data privacy.** This app repo is **public**; real trip data (confirmation codes, stay addresses, daily whereabouts) is never committed to it. Real trips currently live locally in gitignored `private/trips/`, and committed `data/trips/` carries fictional test fixtures that exercise the full schema. **Data home (decided 2026-08-20): a separate private data repo** — `wayfeather-data`, private, holding `data/trips/` with the real itineraries; this public app repo keeps its fictional fixtures as the default/demo data and the test corpus. The app's Settings (owner/repo/PAT) point at the data repo; until they're configured, the app reads its own origin's fixtures. Real trips reach the phones through M2's Contents-API read path — never through this public repo. Hosting: **Cloudflare Pages** either way (free, deploys from public or private repos).
...
**Auth = one fine-grained PAT**, scoped to the repo that holds real trip data, with Contents read/write only. Each phone enters it once in Settings; it lives in `localStorage`. Threat model: whoever holds a phone holds the token — acceptable for two people; rotate the token after each trip. The PAT is never committed and never appears in the deployed bundle. **Guests (2026-08-20):** a third traveler gets their **own** token (same recipe, same single-repo scope); after the trip, revoke *that token* at GitHub — access dies instantly regardless of what's on their phone, and the resident phones are untouched. Settings' "Sign out of GitHub" row wipes the token from a device, with copy that says exactly that — *this device only; real revocation happens at GitHub*. Deleting from a phone you don't hold is a courtesy, not a boundary.
...
Conventions: `id` is a stable slug, never regenerated on edit. **Status axes (schema 2, 2026-08-20 — redesigned from the original flat ten-value enum):** `priority` is just **`must` / `yes` / `maybe` / `skip`** (plus `note`, which is a row *kind* — renders as a text row, no links). Two booleans carry the orthogonal axes: **`reserved`** — booked or paid (★★ Reserved chip, reservation glyph at the time, edit guards); **`callAhead`** — information unverified or a call required (renders the ⚠ Call ahead line). **`phone`** (optional string, `""` default) enables a **Call** button (`tel:` link) in the card's action tiles. Migration from the old enum: `fixed` → `yes` + `reserved:true` (or `must` + `reserved` where it was clearly a must); `maybe-not` / `if-close` / `optional` → `maybe`; `check` → `callAhead:true` on `yes` or `maybe` by context. **Reserved means money or a booking** — travel-logistics rows (Drive legs, pack-and-check-out bookends) migrate to plain `yes`, anchored by their times alone, never ★★. `day` must match a `days[].key`; the special key `bonus` (or any day with `date: null`) renders as an unscheduled section. A reserved cluster name **`Inbox`** holds quick-captured places that haven't been slotted yet (see §6). `lat`/`lng` are nullable; the app works without them (links fall back to address queries) but the solver needs them. Link fields are optional — the renderer already synthesizes Yelp search, Google Maps, Apple Maps, and Google search URLs from name + address, exactly as v0 does (locality derived from `base.address`, never hardcoded). `visited` and `skipped` are nullable ISO timestamps set by the Did it! / Skip it buttons (additive to schema 1; absent means null). A place is *handled* when either is set; `visited` wins for display. The machine-readable version of this section is `data/schema.json` — update both together.

**`cost` is a number as of schema 2 (2026-08-20, the first breaking bump):** dollars as a float — `0` means *known* free, `null` means unknown / varies / not applicable and renders as nothing (never as "Free"). Nuance that used to live in cost text ("StubHub credit", "pay-what-you-wish Wed 1:30–5:30", "$10–20") moves into `notes`; ranges migrate to their upper bound (budget-honest). The card renders cost as a small pill (`$32`, `Free`; no pill when null); the edit sheet takes a numeric input. Retiring cost strings also retires the chronic `$$`-escaping hazard from live data (the code guards stay). `index.json` keeps its own versioning (still 1).
...
**Direction (updated 2026-08-20, v4 avian pass): Soft Cards, light mode only, one theme.** Warm consumer-app aesthetic — cream ground, rounded cards with soft shadows, `-apple-system` stack. **Single palette, "avian matcha"**: mossy matcha green as the one accent ("a strong mossy" — keep it strong), warm brown as the secondary for glyphs and times, and a warm cream ground carrying a *very faint* monotone wallpaper pattern — small bird silhouettes with dashed flight loops, original artwork drawn as an inline SVG tile, sparse and low-contrast; cards sit opaque on top so text contrast never depends on it. Shipped wallpaper (2026-08-20, owner's pick after browsing): **wp-02 "Covey"** — a dense, calm all-over field of small birds, uniform enough that the strips between cards stay consistent; chosen from ten samples in `app/assets/wallpaper-samples/`. The cerulean option and the accent picker are removed. The `color` field stays in trip data but the UI ignores it. **Type borrows the Editorial candidate's split**: serif display via the system serif stack (`ui-serif`, "New York", Georgia, Times fallbacks) for everything that *speaks* — trip name, day titles, stopover names, the day `plan`, notes prose — and `-apple-system` sans for everything that *labels* — times (tabular-nums), meta, hours chips, buttons, tabs, captions. Buttons are rounded rectangles, not pills; forward carats (▸) are drawn as a tiny flying-bird glyph; confetti particles are feather-shaped. **Flashes of antique gold** (a brass/ochre family, tuned dark enough to pass AA wherever it must carry meaning) join as the third and rarest color — reserved for precious or celebratory semantics only: the reservation glyph beside fixed times, the ★ of the Must chip, a scatter of gold feathers in the confetti, the occasional fine celebratory rule. Never structural — no gold borders, grounds, or interaction states at scale; shape carries the meaning, gold decorates it. The identity in one phrase: **elegant ornithological** — a classy field-guide, not a theme park. *(Gold is deferred by decision: it lands in a final UI polish pass after the geographic map ships — not in v4.)* Candidates v1–v3 are frozen references; the avian pass lands on the modular `/app` (M1).

**App icon (decided 2026-08-20):** a small bird standing on top of a piece of vintage luggage — a classic hard-shell suitcase — drawn in an old-illustration style (engraved / field-guide / travel-poster lineage, pick what reads best at 60px), in the avian-matcha palette on cream. Full-bleed square master SVG; iOS masks its own corners. No lettering. **Shipped (2026-08-20): the poster variant, refined r4** (livelier bird, 60px-resolved case fittings, deeper sage disc; refinement sheet in `app/icons/refined/`). The field-guide variant is reserved as the splash / empty-state illustration (`app/assets/fieldguide-illustration.svg`, wiring TBD); all three masters kept in `app/icons/variants/`.
...
**App chrome — bottom tab bar** (dissolves the old menu sheet): **Stopovers · Route · Trips · Settings** (renamed 2026-08-20 from Itinerary/Map — display labels and aria only; internal view keys and `#map`-style deep links stay stable).

- *Stopovers* — the itinerary day view. Segmented day strip at top: adjoined blocks (not pills), large press areas, one finger-sweep spans the trip, completed days greyed (complete = every actionable stopover handled, or the date has passed). Pull-to-refresh (slide down + release) with "updated just now" (+ short sha once the API path exists); the header **Nest** line (birdhouse icon) deep-links to Apple Maps. The day card shows bullet + title, the one-line `subtitle`, an optional longer **`plan`** paragraph (new optional day field, §3), and "N stopovers".
- *Route* — the map tab: the current day as a node line with direction arrows; docked bottom card shows the next unhandled stopover with ◀ ▶ cycling; tapping a node jumps the bar's stopover. **Full-bleed map (2026-08-20):** the map fills the tab edge-to-edge. No day title, no day switcher on this tab — just a very thin floating context pill (e.g. "Fri · Jun 4") over the top; day switching lives on Stopovers. Bottom: a **thin stopover bar** — the time + single-line name span the bar's **full width** on their own row; the ◀ ▶ cycling buttons are **compact (44px square, not full-height)** sitting with the action buttons beneath (2026-08-20: full-height end arrows crowded the title). Compact Landed!/Flew past and a **Details** button complete the bar. **Landed styling on the bar is map-specific and light-touch** — matcha check + accent text on the normal bar ground; the dark journal inversion belongs to itinerary cards and the Details sheet only (the dark bar read odd at 96px). Details opens the standard bottom sheet *over the map* with the full stopover card (the same card fragment the itinerary renders — chosen over jumping back to the itinerary, so map context is never lost). The Route button jumps here. **The geographic map (decided 2026-08-20): MapLibre GL, vendored, + keyless vector tiles** (OpenFreeMap hosted; later a Protomaps PMTiles extract of the trip area shipped as a static file = offline map), **custom-styled to the avian-matcha palette** so the map reads as this app, not an embed. The day's stopovers render as state-colored pins joined by a direction-arrowed travel line in visit order (straight segments first; road-following walking routes are a separate later decision — they'd need a routing service). Requires the Nominatim geocode backfill. **Viewport is derived, never stored**, at three scopes: trip → fit-bounds of all geocoded stopovers (padded); day → fit-bounds of that day's stopovers; cluster ("Walk it") → fit-bounds of the stretch. Fallbacks: a single stopover centers at a walking zoom (~15–16); a day with no coordinates falls back to trip bounds; if the user's location dot is inside the current bounds, fitting never yanks the view away from it. The schematic node view remains the fallback for no-coords and offline-without-extract.
...
- **Time** — larger, top-left. `reserved` stopovers show a small reservation glyph beside the time (mandatory-to-be-there); other times read as planned, not binding.
- **Chips** — **★★ Reserved** (the `reserved` boolean: booked or paid — the loudest mark, alongside the reservation glyph at the time), **★ Must**, and a quiet **Maybe**. `yes` is unmarked; the `callAhead` boolean renders the ⚠ Call ahead line; `skip`/`note` stay row states. Chips and booleans compose — a reserved must shows ★★ and ★ semantics together (★★ wins the corner; the glyph carries reservation). A `phone` value adds a **Call** tile (`tel:`) to the link row.
- **Reserved guard (2026-08-20)** — `reserved` stopovers are *protected, not locked*: changing their time or day, moving them, or skipping them asks one confirmation ("Reserved — change anyway?"); notes/hours/cost/link edits and **Landed!** stay one-tap (you do land at reservations, and plans do sometimes change — the guard is a speed bump that says "this one costs money," not a wall). Find-me-something never relocates a reserved stopover, and the solver already pins them as anchors (§7).
...
- **Clusters** — free text, daypart-led by convention: **Morning / Afternoon / Evening / Twilight**, optionally "— area" (fixtures follow this; the edit sheet's cluster datalist seeds the four dayparts). **Route ▸** on the day card is the single map entry point — the cluster-level "Walk it" button was removed (2026-08-20; the owner asked twice what it did, which was the verdict). The map itself provides stretch focus, and the per-stretch walking-directions link lives inside the route view.
...
**Handled-state contrast (v4.2, supersedes the v4.1 tint):** **Landed goes dark** — the card inverts to the deep warm-brown "journal" treatment lifted from the sample-chooser's entry cards (`app/assets/wallpaper-samples/preview.html` is the styling reference): near-black warm ground, cream serif name, sage/muted labels, matcha check, a thin olive accent bar. A done stopover reads as richly filed away. **Flew past stays the most muted thing on screen** — grey recess, hollow dash marker, dimmed text (the v4.1 treatment). Every string on the dark card still measures ≥4.5:1. Both states keep their Undo.
...
Per-day ordering is a small **TSP with time windows**: n ≤ ~13 stops, reserved anchors (`reserved: true`) pinned to their times, every other stop constrained by parsed open/close windows.
...
Open the repo and start with: *"Read DESIGN.md and CLAUDE.md, then start M1."* Suggested first PRs: (1) split `app/index.html` into modules with zero visual change, (2) data layer (`api.js`: get/put contents, sha handling, IndexedDB cache), (3) menu + trip loader, (4) SW + manifest. Keep `app/index.html` (v0) untouched until M1 renders both trips identically, then delete it in the same PR that proves parity. Testing stays light: Vitest on the pure functions only — hours parser, slug/id, URL builders, solver. Everything UI gets tested the honest way, on an iPhone in standalone mode. **Test suite (lands right after v4):** Vitest as a devDependency only — the app itself stays build-free. Targets: `app/js/time.js` (clock parsing, tz day logic, chronological slotting), `app/js/trip.js` (ordering, clusters, completeness, XTRA synthesis, slug generation/collisions), `app/js/links.js` (URL synthesis, locality derivation), `app/js/dom.js` (escaping, and an explicit `$$`-splice regression test), plus fixture-vs-`data/schema.json` validation so bad data can't land silently.

**Object storage (considered and declined, 2026-08-20):** S3/R2 for trip JSON loses git's free history/blame/rollback/conflict-detection and adds an AWS account, credentials on phones, and a misconfiguration surface — rejected. The future niche where a bucket *does* fit: trip media (photos on stopovers), which doesn't belong in git — R2 behind the M4 Worker when that day comes.
