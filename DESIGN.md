...
- **Chips** — **★★ Reserved** (`fixed`: booked or paid — the loudest mark in the system, returned as a chip 2026-08-20 alongside the reservation glyph at the time), **★ Must**, and a quiet **Maybe** (collapses `maybe`/`maybe-not`/`if-close`/`optional`). `yes` is unmarked; `check` renders the ⚠ Call ahead line; `skip`/`note` stay row states. Schema enum unchanged — `fixed` already meant exactly this.
- **Reserved guard (2026-08-20)** — `fixed` stopovers are *protected, not locked*: changing their time or day, moving them, or skipping them asks one confirmation ("Reserved — change anyway?"); notes/hours/cost/link edits and **Landed!** stay one-tap (you do land at reservations, and plans do sometimes change — the guard is a speed bump that says "this one costs money," not a wall). Find-me-something never relocates a reserved stopover, and the solver already pins them as anchors (§7).
